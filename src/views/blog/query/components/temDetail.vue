...
<template>
  <div class="detailBox tcommonBox">
    ...
    <header>
      <h1>
        <a :href="'#/blog/DetailShare?aid='+detailObj.id" target="_blank">
          {{detailObj.title}}
        </a>
      </h1>
      <h2>
        <i class="fa fa-fw fa-user"></i>发表于 <span>{{create_time}}</span> •
        <i class="fa fa-fw fa-comments"></i>评论 {{detailObj.comment_count}} 条 •
        <span class="rateBox">
                        <i class="fa fa-fw fa-heart"></i>{{detailObj.like_count}}点赞
                    </span>
      </h2>
      <div class="ui label">
        <a :href="'#/Share?platFormId='+detailObj.is_platform">{{detailObj.cate_name}}</a>
      </div>
    </header>
  ...
  export default {
    data() { //选项 / 数据
      return {
        ...
        userId: '',//用户id
        create_time: '2020-09-04T22:30:48+08:00'
      }
    },
    methods: { //事件处理器
      ...
      likecollectHandle: function(islike) {//用户点击喜欢1,用户点击收藏2
        ...
        if (localStorage.getItem('userInfo')) {//判断是否登录
          var tip = ''
          if (islike == 1) {
            console.log('likeart:', that.likeArt)
            if (!that.likeArt) {
              that.likeCount += 1
              that.likeArt = true
              tip = '已点赞'
          ...
          getArtLikeCollect(that.userId, that.aid, islike, that.likeArt, that.collectArt, function(msg) {
          ...
          that.$confirm('登录后即可点赞和收藏，是否前往登录页面?', '提示', {
            ...
            that.$router.push({ path: '/UserLogin?login=1' })
          }).catch(() => {//取消关闭弹窗
      ...
      routeChange: function() {
        ...
        if (localStorage.getItem('userInfo')) {
          ...
          that.username = that.userInfo.user_name

          // console.log(that.userInfo);
          //根据aid，userid 调用接口判断是否已点赞或者是已收藏
          analyzeArtLikeCollect(that.aid, that.userId, function(result) {
            //console.log(result)
            if (result.like_bj) {
              that.likeArt = true
            } else {
        ...
        //记录访客日志
        getGuestLog(that.aid, that.username, function(msg) {
          //console.log("获取返回:",msg)
        })
      }
    },
    watch: {
