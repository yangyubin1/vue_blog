...
<template>
  <div class="rightlistBox">
    <section>
      <div class="r1-head">
        <img src="../../static/img/myheadtou02.jpg" alt="">
        <h1>
          <span>宅男</span>Alex
        </h1>
      </div>
      <div class="r1-body">
        ...
        <div class="catch-me">
          ...
          <div class="">
            ...
            <el-tooltip class="item" effect="dark" content="知乎" placement="top">
              <a :href="catchMeObj[isAimee].zhihu" target="_blank"><i class="">知</i></a>
            </el-tooltip>
            ...
            <el-tooltip class="item" effect="dark" content="更多" placement="top">
              <a href="#/blog/Aboutme"><i class="el-icon-more"></i></a>
            </el-tooltip>
          </div>
        </div>
      </div>
      ...
      <ul class="rs3-textwidget">
        <li class="rs3-item" v-for="(item,index) in artCommentList" :key="'artCommentList'+index">
          <a :href="'#/DetailShare?aid='+item.id" target="_blank">
            <div class="rs3-photo">
              <img :src="base_url + item.avatar" :onerror="$store.state.errorImg">
            </div>
            <div class="rs3-inner">
    ...
    <section class="rs4">
      ...
      <ul>
        <li v-for="(item,index) in browseList" :key="'browseList'+index">
          <a :href="'#/blog/DetailShare?aid='+item.id" target="_blank">{{item.title}}</a> —— {{item.guest_count}} 次围观
        </li>
      </ul>
    </section>
    <!-- 右侧上滑小图片 -->
    <div  :class="gotoTop?'toTop hidden':'toTop goTop hidden'"
         @click="toTopfun" style="border: solid red">
...
<script>
  ...
  import { ShowBrowseCount,ShowArtCommentCount,showLikeData,GetLike} from "../../utils/server"
  export default {
    data() { //选项 / 数据
      return {
        base_url: process.env.VUE_APP_MEDIA_API + 'avatar/',
        fixDo: false,
        loveme: false,
        gotoTop: false,//返回顶部
        ...
        catchMeObj: {//抓住我 个人信息
          ...
          Alex: {
            git: 'https://github.com/yangyubin1',
            qq: 'static/img/message01.jpg',
            sina: 'https://weibo.com/u/6977421172/home?wvr=5',
            wechat: 'static/img/aimee/erwm.jpg',
            zhihu: 'https://www.zhihu.com/people/yang-yu-bin-80-93',
            job: 'https://yangyubin1.github.io/'
          }
        },
        isAimee: 'Alex' //判断是哪个的博客
      }
    },
    methods: {
      lovemeFun:function(){
    ...
    created() { //生命周期函数
      ...
      showLikeData(function(data) {
        that.likeNum = that.initLikeNum = data.like_num
      })
