  ...
  <div class="">
        ...
        <el-col :span="24">
          ...
          <div class="headBox">
            <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect"
                     :router="true">
              <el-menu-item index="/index"><i class="fa fa-wa fa-home"></i> 首页</el-menu-item>
              <el-submenu index="/Share">
                ...
                <el-menu-item v-for="(item,index) in classListObj" :key="'class1'+index"
                              :index="'/blog/Share?classId='+item.platform_id">{{item.platform_desc}}
                </el-menu-item>
              </el-submenu>
              ...
              <el-menu-item index="/blog/Friendslink"><i class="fa fa-wa fa-users"></i> 偶像</el-menu-item>
              <el-menu-item index="/blog/Message"><i class="fa fa-wa fa-pencil"></i> 留言板</el-menu-item>
              <el-menu-item index="/blog/Aboutme"><i class="fa fa-wa fa-vcard"></i> 关于</el-menu-item>
              <el-menu-item index="/blog/write" v-show="write_bj"><i class="fa fa-wa fa-cny"></i> 写微博
              </el-menu-item>

              <div index="" class="pcsearchbox">
                <i class="el-icon-search pcsearchicon"></i>
                <div class="pcsearchinput" :class="input?'hasSearched':''">
                  <el-input placeholder="搜索文章" icon="search" v-model="input" :on-icon-click="searchEnterFun"
    ...
    <div class="headImgBox">
      ...
      <div class="h-information">
        <a href="#/blog/Aboutme">
          <img src="../../static/img/mytou.png" alt="">
        </a>
        <h2 class="h-description">
          <a href="#/blog/Aboutme">
            "小乌龟站在大佬的肩膀上，好好学习，天天向上"
          </a>
        </h2>
      </div>
    </div>
...
<script>
  import {
    LoginOut, ArtClassData, navMenList, AboutMeData
  ...
  export default {
    data() { //选项 / 数据
      return {
        ...
        // path:'',//当前打开页面的路径
        input: '', //input输入内容
        headBg: 'url(static/img/myheadbg05.jpg)', //头部背景图
        headTou: '', //头像
        projectList: '' //项目列表
      }
    },
    ...
    created() { //生命周期函数
      ...
      // var usr_info = localStorage.getItem('user_info')
      //
      // console.log("user_info:",user_info)
      //
      // //更新write_bj
      // if (usr_info && user_info.get('user_name') == 'alex.yang') {
      //   that.write_bj = false
      // }
      ...
      var onVisibilityChange = function() {
        if (document[hiddenProperty]) { //被隐藏
          document.title = '球进啦(つд⊂)'
        } else {
          document.title = '任意球，梅西！(*´∇｀*)' //当前窗口打开
    ...
    methods: { //事件处理器
      ...
      searchEnterFun: function(e) { //input 输入 enter
        ...
        if (this.input) {
          this.$store.state.app.keywords = this.input
          console.log("关键词:",this.$store.state.app.keywords)
          this.$router.push({ path: '/blog/Share?keywords=' + this.input })
        }
      },
      ...
      init_data: function() {
        ...
        //console.log('获取userinfo', localStorage.getItem('userInfo'))
        if (localStorage.getItem('userInfo')) { //存储用户信息
          that.haslogin = true
          that.userInfo = JSON.parse(localStorage.getItem('userInfo'))
          //console.log(that.userInfo);
          if (that.userInfo.user_name == 'alex.yang') {
            that.write_bj = true
          }
        } else {
          that.haslogin = false
      ...
      logoinFun: function(msg) { //用户登录和注册跳转
        ...
        if (msg == 0) {
          this.$router.push({
            path: '/UserLogin?login=0'
          })
        } else {
          this.$router.push({
            path: '/UserLogin?login=1'
          })
        }
      },
      userlogout: function() {
...
<style lang="scss">
  ...
  .headImgBox {
    ...
    margin-bottom: 90px;
    background: url(../../static/img/myheadbg05.jpg);
  }
