...
<template>
  <div>
    <div class="container">
      <h1 class="loginTitle">
        <a href="#/" style=" color: #ffffff;">{{this.$store.getters.themeObj==0 ?'Alex的小窝':'路人甲的博客'}}</a>
      </h1>
      <!-- 登录注册 -->
      <div v-show="!err2005" >
        <div v-if="login==1" class="loginBox">
          <div class="lr-title">
            <h1>登录</h1>
            <p>
              新用户<a href="#/UserLogin?login=0" class="tcolors">注册</a>
            </p>
          </div>
          <el-alert
            v-show="loginErr"
        ...
        <div v-else class="registerBox">
          <div class="lr-title">
            <h1>注册</h1>
            <p>
              已有账号<a href="#/UserLogin?login=1" class="tcolors">登录</a>
            </p>
          </div>
          <el-alert
            v-show="registerErr"
      ...
      <div v-show="err2005" class="registerSuc" >
        ...
        <div v-show="urlstate==0" class="sucContent">
          ...
          <p style="color:#ffffff">请您在24小时内登录邮箱，按邮件中的提示完成账号激活操作</p>
        </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      //console.log('获取的store:', this.$store.getters.themeObj)
    },
      ...
      gotoHome: function() {//用户登录
        ...
        if (!that.emailErr && !that.passwordErr) {
          UserLogin(that.email, that.password, function(msg) {
            ...
            } else if (msg.code == 20001) {//邮箱或密码错误
              that.loginErr = true
              that.loginTitle = '邮箱或密码错误'
            } else if (msg.code == 20002) {//邮箱注册码未激活
              that.loginErr = true
              that.loginTitle = '该邮箱注册码未激活，请前往邮箱激活'
            } else if (msg.code == 20003) {//邮箱注册码未激活已超时
              // that.loginErr = true;
              that.err2005 = true
