          ...
          <div class="OwO-body">
            <ul class="OwO-items OwO-items-show">
              <li class="OwO-item" v-for="(oitem,index) in OwOlist" :key="'oitem'+index"
                ...
                <!--          <img src="../../../../static/img/emot/image/weixiao.gif" alt="">-->
                <!--                <img src="@/assets/emot/image/weixiao.gif" alt="">-->
          ...
          <li class="tmsg-c-item" v-for="(item,index) in commentList" :key="'common'+index">
            ...
            <ul v-show="item.ChildsSon" class="tmsg-commentlist" style="padding-left:60px;">
              <li class="tmsg-c-item" v-for="(citem,cindex) in item.ChildsSon" :key="'citem'+cindex">
                <article class="">
                  <header>
                    <img :src="base_url +citem.avatar" :onerror="$store.state.errorImg">
                    <div class="i-name">
                  ...
                  <section>
                    <p v-html="analyzeEmoji(citem.content)">{{analyzeEmoji(citem.content)}}</p>
  ...
  export default {
    data() { //选项 / 数据
      return {
        ...
        hasMore: true,
        haslogin: false,
        userId: '20202020',//用户id
        leaveId: 0,//回复评论的当前的commentId
        leavePid: '',//赞赏等其他模块的分类id
        pid: 0,//回复评论的一级commentId(0默认为顶级）
        sendTip: '发送~',
        OwOlist: [//表情包和表情路径
    ...
    methods: { //事件处理器
      ...
      analyzeEmoji: function(cont) {//编译表情替换成图片展示出来  正则匹配汉子，然后替换成表情的图片
        var pattern1 = /\[[\u4e00-\u9fa5]+\]/g
        var pattern2 = /\[[\u4e00-\u9fa5]+\]/
      ...
      sendMsg: function() {//留言
        ...
        if (that.textarea) {
          ...
          if (that.leaveId == 0) {
            ...
            setArticleComment(that.textarea, that.userId, that.aid, that.pid, that.avatar, that.user_name, that.label, function(msg) {
              //console.log("获得返回:",msg);
      ...
      showCommentList: function(initData) {//评论列表
        ...
        //判断当前用户是否登录
        // console.log("缓存获取的用户信息:", localStorage.getItem('userInfo'))
        if (localStorage.getItem('userInfo')) {
          that.haslogin = true
          that.userInfo = JSON.parse(localStorage.getItem('userInfo'))
          that.userId = that.userInfo.userId
          that.avatar = that.userInfo.avatar
          that.label = that.userInfo.label
          that.user_name = that.userInfo.user_name
        ...
        if (that.$route.name == 'DetailShare') {//文章列表的评论
          ...
          ArticleComment(0, that.pageId, function(result) {//查询列表
            setData(result)
          })
