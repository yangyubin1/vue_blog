        ...
        <el-col :span="24" class="s-item tcommonBox" v-for="(item,index) in articleList" :key="'like'+index">
          ...
          <div class="article-content">
            ...
            <p style="max-height:300px;overflow:hidden;text-align:center;">
              <img :src="baseUrl + item.image_url" alt="" class="maxW">
            </p>
          </div>
  ...
  export default {
    ...
    methods: { //事件处理器
      ...
      cancelLikeCollect: function(id) {
        var that = this
        // console.log(id);
        let likeArt = that.like == 1 ? false : true
        let collectArt = that.like == 1 ? true : false
        getArtLikeCollect(that.userId, id, that.like, likeArt, collectArt, function(msg) {
      ...
      showLikeCollectList: function(initpage) {//展示数据
        ...
        getLikeCollectList(that.userId, that.artId, that.like, (result) => {
          if (result.code == 20000) {
            var msg = result.data
            // console.log(result.data);
            if (result.total > 0 && result.total <= 3) {
              that.hasMore = false
            } else {
              that.hasMore = true
            }
